...
class LocalMCPBridgeExecutor:
    ...
    @staticmethod
    def _validate_job_payload(payload: Any) -> dict[str, Any]:
        if not isinstance(payload, dict):
            raise BridgeExecutionAdapterError(
                "Runner MCP returned an invalid test job result"
            )
        ...
        if not isinstance(status, str):
            raise BridgeExecutionAdapterError(
                "Runner MCP returned an invalid test job status"
            )
        return payload

    def run_tests(self, project: str, suite: str) -> Any:
        return self._validate_job_payload(
            self._client()._call_tool(
                "run_tests",
    ...
    def job_status(self, job_id: str) -> Any:
        return self._validate_job_payload(
            self._client()._call_tool("job_status", {"job_id": job_id})
        )

    def cancel_job(self, job_id: str) -> Any:
        return self._validate_job_payload(
            self._client()._call_tool("cancel_job", {"job_id": job_id})
        )
